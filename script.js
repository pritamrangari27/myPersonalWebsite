...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const typedTextSpan1 = document.getElementById("hero-typed-1");
    const typedTextSpan2 = document.getElementById("hero-typed-2");
    const cursorSpan = document.getElementById("hero-cursor");

    if (typedTextSpan1 && typedTextSpan2 && cursorSpan) {
        const text1 = "Hi, I'm ";
        const text2 = "Pritam Rangari";
        const typingDelay = 180; // Slowed down from 100ms
        let charIndex = 0;
        ...
        function type() {
            if (!isTypingText2) {
                if (charIndex < text1.length) {
                    typedTextSpan1.textContent += text1.charAt(charIndex);
                    charIndex++;
                ...
                if (charIndex < text2.length) {
                    typedTextSpan2.textContent += text2.charAt(charIndex);
                    charIndex++;
                    setTimeout(type, typingDelay);
                } else {
        ...
        setTimeout(type, 500); // Start after 500ms
    }
    ...
    if (chatbotToggle && chatbotWindow) {
        chatbotToggle.addEventListener('click', () => {
            ...
            if (!isActive) {
                // Fresh chat: clear history
                chatbotMessages.innerHTML = `
                    <div class="chat-message bot-message">
                        Hi! I'm Pritam's virtual assistant. How can I help you today?
                    </div>`;
            }
            chatbotWindow.classList.toggle('active');
            chatbotToggle.style.transform = !isActive ? 'scale(0)' : 'scale(1)';
        });
        ...
        const sendMessage = async () => {
            ...
            typingMsg.className = 'chat-message bot-message typing-indicator';
            ...
            try {
                ...
                botMsg.textContent = data.reply || "I didn't quite get that.";
                ...
                errorMsg.textContent = "Sorry, I am having trouble connecting to my brain right now.";
